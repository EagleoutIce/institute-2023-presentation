...
\begin{frame}
   \begin{tikzpicture}[@O]
      \node[font=\huge\bfseries,above right=7.5mm] (@) at (current page.south west) {\insertsection};
...
\begin{tikzpicture}[@O]
   \node[above left=1mm,font=\tiny\sbfamily,yshift=2pt,xshift=-.33pt] at(current page.south east) {\thepage};
\end{tikzpicture}%
}

\usepackage{DejaVuSansMono}
\makeatletter
\solSetStyle{basic}{\sol@ttfamily\sbfamily}%


\NewEnviron{focus}{%
   \begin{layout-full}\begin{tikzpicture}[@O]
      \node[text width=\linewidth,align=center] at(current page.center) {\BODY};
